...
import java.io.*;
import java.util.ArrayList;  // Import IO classes for file handling
...
public class ImprovedTextSearch extends JFrame {
    ...
    public ImprovedTextSearch() {
        ...
        createComponents(); // Creating the GUI components
        arrangeComponents();  // Arrange components in the window / layout
        setupEventHandlers(); // Setting up event handlers for buttons and actions
    }
...
// Arrange GUI components into panels
    private void arrangeComponents() {
        JPanel topPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        topPanel.setOpaque(false);
        topPanel.add(openButton);
    // Middle panel for search controls
 ...
 // Bottom panel for navigation buttons and status
        ...
        JPanel mainPanel = new JPanel(new BorderLayout(10, 10)) {
            ...
            protected void paintComponent(Graphics g) {
                ...
                // Apply a dark overlay with more transparency (design choice)
    ...
    private void setupEventHandlers() {
        openButton.addActionListener(e -> openFile()); // Open file dialog
        searchButton.addActionListener(e -> findMatches()); // Perform the search
        nextButton.addActionListener(e -> showNextMatch()); // to show the next match
        prevButton.addActionListener(e -> showPreviousMatch()); // to show the previous match
        searchField.addActionListener(e -> findMatches()); // Enter key to triggers search
    }
...
// for reading the file line by line
 ...
 // this is to perform search for the texts
...
// Adjust text depending on case sensitivity
...
// Check if a match is a whole word
    private boolean isWholeWord(int position, int length) {
        ...
        if (position + length < fileContent.length()) {
            ...
            if (Character.isLetterOrDigit(after) || after == '_') {
                return false; // This will be for the Part of a larger word
            }
        }
    ...
    private void updateSearchResults() {
        if (matchPositions.isEmpty()) {
            ...
            showNextMatch(); // Show first match
        }
    }
    ...
    private void showPreviousMatch() {
        ...
        if (currentMatchIndex < 0) {
            currentMatchIndex = matchPositions.size() - 1; // Loop back to last match
        }
        ...
        if (currentMatchIndex == 0) {
            prevButton.setEnabled(false); // Disable when first match is reached
        }
    }
 ...
 // Reset search state
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            searcher.setVisible(true); // Display window
        });
    }
}